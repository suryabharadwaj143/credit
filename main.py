...
class QueryIn(BaseModel):
    ...
    Credit_1: int
    Installment_1: int
    Presentresident_1: int
    Age_1: int
    credit_1: int
    Numberofpeoplebeingliable_1: int
...
def ping():
    return {"hackathon": "successful"}


@app.post("/cs1", response_model=QueryOut, status_code=200)
# Route to do the prediction using the ML model defined.
# Payload: QueryIn containing the parameters
# Response: QueryOut containing the risk class predicted (200)
def cs1(query_data: QueryIn):

    return {"Cost_M_R1": predict(query_data)}

@app.post("/f1_back_loop", status_code=200)
# Route to further train the model based on user input in form of feedback loop
# Payload: FeedbackIn containing the parameters and correct Cost Matrix(Risk) class
# Response: Dict with detail confirming success (200)
def f1_back_loop(data: List[FeedbackIn]):
    retrain(data)
